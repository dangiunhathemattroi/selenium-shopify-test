...
export async function quantityInput(driver, quantity = 1) {
    try {
        ...
        let quantityInput = await driver.findElement(By.css(".product-form__input quantity-input.quantity input.quantity__input"));
        await driver.sleep(2000);
...
export async function quantityInputCart(driver, quantity = 1) {
    try {
        await driver.sleep(2000);
